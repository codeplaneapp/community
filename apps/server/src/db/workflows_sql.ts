...
export async function createWorkflowTask(sql: Sql, args: CreateWorkflowTaskArgs): Promise<CreateWorkflowTaskRow | null> {
    const rows = await sql.unsafe(createWorkflowTaskQuery, [args.workflowRunId, args.workflowStepId, args.repositoryId, args.status, args.priority, args.payload, args.availableAt, args.vmId]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
export const markWorkflowTaskVMRunningQuery = `-- name: MarkWorkflowTaskVMRunning :execrows
UPDATE workflow_tasks
SET status = 'running',
    vm_id = $1,
...
export interface MarkWorkflowTaskVMRunningArgs {
    vmId: string | null;
    id: string;
}
