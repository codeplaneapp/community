...
export function createCodeplaneContextTool(
  ...
  return {
    ...
    description:
      "Return the current local JJ/Codeplane repo context, and optionally refresh it if state may have changed.",
    promptSnippet:
      "codeplane_repo_context(refresh?) returns the current repo root, detected Codeplane repo, jj status, remotes, auth state, and backend details.",
    promptGuidelines: [
      "Use codeplane_repo_context(refresh=true) if local repo state or auth may have changed during the session.",
    ],
