...
export function useScreenLoading(
  ...
  // Register retry callback with the LoadingProvider so global R key can trigger it
  useEffect(() => {
    loading.setRetryCallback(retry);
    return () => {
      loading.setRetryCallback(null);
    };
  }, [loading, retry]);
