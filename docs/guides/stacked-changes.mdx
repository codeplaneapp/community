...
description: Working with stacked changes in Codeplane
---

# Stacked Changes

Stacked changes are Codeplane's premiere feature and a core reason to use jj over git. They're essential for AI workflows, where agents naturally produce incremental, dependent changes at machine speed.
...
Or use the Codeplane CLI to see changes:

```bash
codeplane change list
codeplane change show qzknlmop
codeplane change diff qzknlmop
```

## Pushing a Stack

Push all bookmarks (and the stack) to Codeplane:
...
6. The stack lands as a unit via `codeplane land land`

This is the workflow Codeplane is built for - agents working at speed, humans reviewing at the plan level, and the platform handling the mechanics.
