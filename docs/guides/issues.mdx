---
title: Issues
description: Create, manage, and track issues on Codeplane
icon: circle-exclamation
---

# Issues

Issues are Codeplane's primary tool for tracking bugs, feature requests, tasks, and discussions. They support labels, milestones, assignees, reactions, dependencies, pinning, locking, and templates -- the full feature set you expect from a modern forge.
...
```bash
codeplane issue create --title "Login page returns 500 on Safari" --body "Steps to reproduce..."
```
...
```bash
# Simple issue
codeplane issue create -t "Add dark mode support"
...
codeplane issue create --template bug-report

# Issue with dependencies
codeplane issue create -t "Deploy v2.0" --blocked-by 15 --blocked-by 16
```

### Via API

```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/issues \
...
Issue templates provide structured forms for consistent issue creation. Templates are Markdown files in `.codeplane/ISSUE_TEMPLATE/`:
...
codeplane issue create --template bug-report --title "Safari crash on login"
```
...
# List all issues
codeplane issue list --state all

# Limit results
codeplane issue list --limit 10

# List pinned issues only
codeplane issue list --pinned

# View a specific issue
codeplane issue view 42

# JSON output for scripting
codeplane issue list --json number,title,state
```
...
codeplane issue edit 42 -t "Updated title" -b "Updated description"

# Add an assignee
codeplane issue edit 42 -a bob

# Close an issue
codeplane issue close 42

# Close with a comment
codeplane issue close 42 --comment "Fixed in LR #15"
```
...
# Create a label
codeplane label create bug --color e11d48 --description "Something isn't working"

# Create a label with default color
codeplane label create enhancement

# Delete a label
codeplane label delete bug
```
...
curl -X POST https://api.codeplane.app/api/repos/owner/repo/issues/42/labels \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{"labels": ["bug", "high-priority"]}'

# Remove a label
curl -X DELETE https://api.codeplane.app/api/repos/owner/repo/issues/42/labels/bug \
  -H "Authorization: token codeplane_xxxxx"
```
...
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/owner/repo/milestones

# Create a milestone
curl -X POST https://api.codeplane.app/api/repos/owner/repo/milestones \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "v1.0 Release",
...
codeplane issue create -t "Add auth middleware" --blocks 5

# Create an issue blocked by issues #3 and #4
codeplane issue create -t "Deploy to production" --blocked-by 3 --blocked-by 4

# Add a dependency to an existing issue
codeplane issue edit 7 --blocks 10
codeplane issue edit 7 --blocked-by 2
```
...
codeplane issue deps 7

# JSON output
codeplane issue deps 7 --json
```
...
```bash
# Pin an issue
codeplane issue pin 42

# Unpin an issue
codeplane issue unpin 42
...
```bash
# Lock an issue
codeplane issue lock 42

# Lock with a reason
codeplane issue lock 42 --reason resolved

# Unlock an issue
codeplane issue unlock 42
```
...
curl -X POST https://api.codeplane.app/api/repos/owner/repo/issues/42/comments \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
...
```bash
# Edit a comment
curl -X PATCH https://api.codeplane.app/api/repos/owner/repo/issues/comments/15 \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{"body": "Updated comment text"}'

# Delete a comment
curl -X DELETE https://api.codeplane.app/api/repos/owner/repo/issues/comments/15 \
  -H "Authorization: token codeplane_xxxxx"
```
