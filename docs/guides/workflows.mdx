...
Codeplane workflows are TSX files that define automated tasks for your repository. They use [Smithers](https://smithers.sh) JSX components wrapped by the `@codeplane-ai/workflow` package. There is no distinction between CI workflows and AI workflows -- a workflow task can run tests, deploy code, or invoke an AI agent. They are all just code running in the same sandboxed environment.

Workflows live in your repository under `.codeplane/workflows/` and are triggered by events like pushes, landing request activity, schedules, or manual dispatch.
...
Workflows must be placed in the `.codeplane/workflows/` directory at the repository root:

```
your-repo/
├── .codeplane/
│   ├── package.json          # Depends on @codeplane-ai/workflow
...
│       └── nightly.tsx
├── packages/
│   └── workflow/             # @codeplane-ai/workflow package
│       ├── package.json
│       ├── tsconfig.json
│       └── src/
│           ├── index.ts      # Re-exports Smithers components + triggers
│           ├── components.tsx # Workflow, Task (Codeplane extensions)
│           └── triggers.ts   # on.push(), on.landingRequest.*, etc.
├── package.json              # workspaces: ["packages/*", ".codeplane"]
├── src/
└── ...
```

Each `.tsx` file in the `workflows/` directory is discovered automatically. The filename (without extension) becomes the workflow ID used in CLI commands like `codeplane workflow run ci`.

The `.codeplane/` directory is a Bun workspace member with its own `package.json` and `tsconfig.json`, giving full LSP support (autocomplete, type checking, go-to-definition) when editing workflows. See [Your First Workflow](/getting-started/first-workflow#project-setup) for the complete setup.
...
```bash
codeplane workflow run deploy --input environment=production --input version=1.2.3
```

Or via the API:

```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/workflows/deploy/dispatches \
...
An agent task invokes an AI agent by passing a string prompt as the child and an `agent` prop. The agent runs in the same sandboxed environment and can read/write files, execute commands, and interact with the Codeplane API:
...
```bash
printf %s "sk-ant-..." | codeplane secret set ANTHROPIC_API_KEY --body-stdin
```
...
```tsx
import { createSmithers, Task, on } from "@codeplane-ai/workflow";
...
| Workflow/Task | Imported directly from `@codeplane-ai/workflow` | Destructured from `createSmithers()` |
...
```tsx
import { Workflow, Task, Parallel, on } from "@codeplane-ai/workflow";
import { $ } from "bun";
...
Workflows often need credentials (API keys, deploy tokens) and configuration values. Codeplane provides two mechanisms:
...
# Set a deploy token
printf %s "your-deploy-token" | codeplane secret set DEPLOY_TOKEN --body-stdin

# List secrets (names only, values are hidden)
codeplane secret list

# Delete a secret
codeplane secret delete DEPLOY_TOKEN
```
...
<Warning>
  Secrets are encrypted at rest and only injected into workflow runs. They are never visible in API responses, CLI output, or logs. If a workflow task prints a secret value to stdout, Codeplane automatically redacts it in the log output.
</Warning>
...
```bash
# Set a variable
codeplane variable set NODE_VERSION -b "20"

# Get a variable value
codeplane variable get NODE_VERSION

# List variables
codeplane variable list

# Delete a variable
codeplane variable delete NODE_VERSION
```
...
```tsx
// .codeplane/workflows/deploy.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
import { $ } from "bun";
...
```bash
codeplane workflow run deploy -i environment=production -i version=1.2.3
```
...
```tsx
// .codeplane/workflows/triage.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
import { ToolLoopAgent } from "ai";
import { anthropic } from "@ai-sdk/anthropic";
...
```tsx
// .codeplane/workflows/ai-review.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
import { ToolLoopAgent } from "ai";
import { anthropic } from "@ai-sdk/anthropic";
import { $ } from "bun";
...
```tsx
// .codeplane/workflows/smart-ci.tsx
import { Workflow, Task, Branch, on } from "@codeplane-ai/workflow";
import { ToolLoopAgent } from "ai";
import { anthropic } from "@ai-sdk/anthropic";
import { $ } from "bun";
...
```bash
# Trigger manually
codeplane workflow run ci --ref main

# Trigger with inputs
codeplane workflow run deploy -i environment=staging -i version=1.0.0
```
...
codeplane run list ci

# View a specific run
codeplane run view 42

# Watch a run in real-time (SSE streaming)
codeplane run watch 42

# Re-run a failed workflow
codeplane run rerun 42
```
...
codeplane status list <change-id>

# Set a status manually
codeplane status set <sha> --state success --context ci/test
```
