...
Issue templates are Markdown files stored in the `.codeplane/ISSUE_TEMPLATE/` directory at the root of your repository. Each file defines a single template with YAML frontmatter for metadata and Markdown content for the issue body.
...
Create a file at `.codeplane/ISSUE_TEMPLATE/bug-report.md`:
...
- Codeplane CLI version: [e.g., 0.1.0]
...
**Feature request** (`.codeplane/ISSUE_TEMPLATE/feature-request.md`):
...
**Documentation issue** (`.codeplane/ISSUE_TEMPLATE/docs.md`):
...
Use the `--template` flag (short: `-T`) with `codeplane issue create` to use a template:
...
codeplane issue create --template bug-report --title "Login page crashes on Safari"

# List available templates first
curl -H "Authorization: token codeplane_YOUR_TOKEN" \
  https://api.codeplane.app/api/v1/repos/owner/repo/issue-templates
```

The `--template` flag accepts the template filename without the `.md` extension. For example, a template at `.codeplane/ISSUE_TEMPLATE/bug-report.md` is referenced as `bug-report`.
...
The endpoint reads template files from the `.codeplane/ISSUE_TEMPLATE/` directory on the repository's default bookmark, parses the YAML frontmatter, and returns the structured template data.

## Directory Structure

Templates must be placed in the `.codeplane/ISSUE_TEMPLATE/` directory at the repository root:
