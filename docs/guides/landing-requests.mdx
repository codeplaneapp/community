...
description: Codeplane's jj-native replacement for Pull Requests
---

# Landing Requests

Landing Requests (LRs) are Codeplane's native replacement for Pull Requests. They're built from the ground up for jj's stacked changes model - stable Change IDs, first-class conflict tracking, and automatic rebasing.

## Landing Requests vs Pull Requests

| Aspect | Pull Requests (GitHub) | Landing Requests (Codeplane) |
|--------|----------------------|------------------------|
...
```bash
codeplane land create --title "Add user authentication" --target main
```
...
codeplane land create --title "User authentication" --target main --stack
```
...
| Visible in default `codeplane land list` | No (use `--state draft` or `--state all`) | Yes |
| Can receive reviews | Yes (but no notifications are sent) | Yes |
| Can be merged | No (must be published first) | Yes |

Draft LRs appear with a `draft` state badge when viewed with `codeplane land view`.
...
```bash
codeplane land create --title "Add user authentication" --target main --draft
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/landings \
  -H "Authorization: token codeplane_xxxxx" \
...
**Using `codeplane land ready`:**

```bash
codeplane land ready 42
```
...
**Using `codeplane land edit`:**

```bash
codeplane land edit 42 --state open
```

**Via the API:**

```bash
curl -X PATCH https://api.codeplane.app/api/repos/owner/repo/landings/42 \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{"state": "open"}'
```
...
```bash
codeplane land edit 42 --state draft
```
...
# All states
codeplane land list --state all

# Filter by state: open, closed, merged, draft
codeplane land list --state merged --limit 10
```

## View a Landing Request

```bash
codeplane land view 42
```
...
```bash
# Approve
codeplane land review 42 --approve --body "Looks good"

# Request changes
codeplane land review 42 --request-changes --body "Please add tests"

# Comment only
codeplane land review 42 --comment --body "Question about the approach"
```

## Suggested Changes

Suggested changes let reviewers propose specific code edits inline within a review comment. Instead of describing what to change in plain text, a reviewer can include the exact replacement code. The landing request author can then accept the suggestion with a single command, and Codeplane automatically creates a new change with the edit applied.

This is Codeplane's equivalent of GitHub's suggested changes feature, adapted for jj's change model.
...
Because Codeplane is jj-native, accepting a suggestion creates a new jj change (with a stable Change ID) rather than amending an existing commit. This preserves the full history of how the code evolved during review.
...
codeplane land review 42 --comment --body 'The error message should be more descriptive.
...
When creating a review comment via the API, attach the suggestion to a specific file path and line range so Codeplane knows which lines the replacement targets.
...
codeplane land suggestion list 42
```
...
codeplane land suggestion accept 42 --id 7

# Reject a suggestion by ID
codeplane land suggestion reject 42 --id 7

# Accept all pending suggestions on a landing request at once
codeplane land suggestion accept-all 42
```

When a suggestion is accepted:

1. Codeplane applies the code replacement to the target file at the specified lines.
...
codeplane land suggestion accept-all 42 --squash
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/landings/42/suggestions \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PUT https://api.codeplane.app/api/repos/owner/repo/landings/42/suggestions/7/accept \
  -H "Authorization: token codeplane_xxxxx"
```
...
When a suggestion is accepted, Codeplane performs the following operations on the repo-host:
...
Suggested changes are particularly powerful when combined with Codeplane's AI agent system. An AI agent reviewing a landing request can propose precise code fixes as suggestions rather than just describing issues in prose. The author can review the agent's suggestions and accept them with a single command, creating a tight human-in-the-loop workflow:
...
codeplane land suggestion accept 42 --id 12
codeplane land suggestion accept 42 --id 13
codeplane land suggestion reject 42 --id 14
```
...
Use `codeplane land conflicts` to see the conflict details for a landing request:
...
codeplane change show <change-id>
```

The `codeplane change show` command displays the full conflict object, including the base, left, and right file contents so you can understand exactly what diverged.

### Viewing conflicts via the API

```bash
# Get conflict status for a landing request
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/:owner/:repo/landings/42/conflicts
```
...
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/:owner/:repo/changes/:change_id/conflicts
```

### The conflict resolution workflow

1. **Detect** -- The LR shows `conflict_status: "conflicted"` automatically. You can also see it when running `codeplane land view 42`.

2. **Inspect** -- Run `codeplane land conflicts 42` to see which files are conflicted and which changes are involved. Then use `codeplane change show <change-id>` to see the base, left, and right versions.
...
Use `codeplane land thread resolve` to mark a review comment thread as resolved, and `codeplane land thread unresolve` to re-open it:

```bash
# Resolve a review comment thread
codeplane land thread resolve <comment-id>

# Re-open a previously resolved thread
codeplane land thread unresolve <comment-id>
```
...
To see review threads on a landing request, use `codeplane land threads`. By default this shows all threads; use `--unresolved` to filter to only unresolved ones:

```bash
# List only unresolved threads on LR #42
codeplane land threads 42 --unresolved

# List all threads (both resolved and unresolved)
codeplane land threads 42
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/landings/42/comments/15/resolve \
  -H "Authorization: token codeplane_xxxxx"
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/landings/42/comments/15/unresolve \
  -H "Authorization: token codeplane_xxxxx"
```
...
```bash
codeplane bookmark protect main --require-resolved-conversations
```

When this rule is active and unresolved conversations remain, `codeplane land land` will fail with a clear error:

```bash
$ codeplane land land 42
...
The `codeplane land view` command also shows the count of unresolved conversations, so authors can track their progress toward satisfying this protection rule.
...
```tsx
// .codeplane/workflows/landing-queue.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
import { $ } from "bun";
...
Use `codeplane bookmark protect` to configure protection on a bookmark pattern:

```bash
# Protect the main bookmark - require 1 approval before landing
codeplane bookmark protect main --require-review --approvals 1

# Protect all release bookmarks with 2 required approvals
codeplane bookmark protect "release/*" --require-review --approvals 2

# Require status checks to pass before landing
codeplane bookmark protect main --require-status-checks

# Combine multiple rules
codeplane bookmark protect main \
  --require-review \
  --approvals 2 \
...
```bash
codeplane bookmark unprotect main
```

To list all protection rules for a repository:

```bash
codeplane bookmark protections
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/bookmark-protections \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "pattern": "main",
...
When you run `codeplane land land` on a landing request that targets a protected bookmark, Codeplane enforces all matching protection rules:
...
3. **Resolved conversations** - If `require_resolved_conversations` is enabled, all review comment threads on the LR must be resolved before landing. Use `codeplane land threads 42 --unresolved` to see remaining threads.
...
The `codeplane land view` and `codeplane land checks` commands also display protection rule status, so authors know exactly what is needed before attempting to land.

---

## Lifecycle

1. **Create** - Author opens an LR with `codeplane land create` (optionally as a `--draft`)
1. **Draft (optional)** - If created as a draft, iterate on changes without triggering notifications. Publish with `codeplane land ready` when ready.
2. **Review** - Team reviews with `codeplane land review`. Reviewers can leave comments, approve, request changes, or propose [suggested changes](#suggested-changes) with inline code replacements.
3. **Apply Suggestions** - Author reviews any suggested changes and accepts or rejects them with `codeplane land suggestion accept` / `reject`. Accepted suggestions automatically create new changes in the stack.
4. **CI Checks** - Automated workflows validate the changes
5. **Resolve Conflicts** - If conflicts arise, view them with `codeplane land conflicts`, resolve locally with `jj resolve`, and push. The LR updates automatically.
6. **Resolve Conversations** - Address review feedback and mark threads as resolved with `codeplane land thread resolve`. If bookmark protection requires resolved conversations, all threads must be resolved before landing.
7. **Protection Rules** - All bookmark protection rules for the target bookmark must be satisfied
8. **Land** - Author or maintainer lands with `codeplane land land`
...
Reactions let you add emoji responses to issues and comments. Codeplane supports the same reaction types as Gitea: `+1`, `-1`, `laugh`, `hooray`, `confused`, `heart`, `rocket`, and `eyes`.
...
codeplane issue react 42 +1

# List reactions on an issue
codeplane issue reactions 42

# Add a reaction to an issue comment (by comment ID)
codeplane issue react --comment 15 heart
```
...
```bash
# List reactions on an issue
curl -H "Authorization: token $CODEPLANE_TOKEN" \
  https://api.codeplane.app/repos/{owner}/{repo}/issues/{number}/reactions

# Add a reaction to an issue
curl -X POST -H "Authorization: token $CODEPLANE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"content": "+1"}' \
  https://api.codeplane.app/repos/{owner}/{repo}/issues/{number}/reactions

# Remove a reaction from an issue
curl -X DELETE -H "Authorization: token $CODEPLANE_TOKEN" \
  https://api.codeplane.app/repos/{owner}/{repo}/issues/{number}/reactions/{id}
```
...
curl -X POST -H "Authorization: token $CODEPLANE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"content": "rocket"}' \
  https://api.codeplane.app/repos/{owner}/{repo}/issues/comments/{id}/reactions
...
curl -X DELETE -H "Authorization: token $CODEPLANE_TOKEN" \
  https://api.codeplane.app/repos/{owner}/{repo}/issues/comments/{id}/reactions/{reaction_id}
```
