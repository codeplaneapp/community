...
Codeplane supports two signing backends:
...
Both backends produce signatures that Codeplane can verify when the corresponding public key is uploaded to your account.
...
SSH signing uses the same key format as your authentication SSH keys. If you already have an SSH key uploaded to Codeplane for push/pull access, you can use that same key for signing.

## Uploading Your Signing Key to Codeplane

For Codeplane to verify your signed commits, you must upload the corresponding public key to your account.
...
codeplane gpg-key add --key-file ~/.gnupg/pubkey.asc

# Or pipe the key directly
gpg --armor --export YOUR_GPG_KEY_ID | codeplane gpg-key add
```

**Upload via API:**

```bash
curl -X POST https://api.codeplane.app/api/user/gpg-keys \
...
```bash
codeplane gpg-key list
```

**Delete a GPG key:**

```bash
codeplane gpg-key delete <key-id>
```

### SSH Keys

SSH signing keys use the same keys you upload for SSH authentication. If you have already added an SSH key via `codeplane ssh-key add`, that key is automatically available for commit signature verification -- no additional upload is needed.

If you use a separate key for signing (different from your authentication key), add it as an SSH key:

```bash
codeplane ssh-key add -t "Signing Key" -k "$(cat ~/.ssh/id_ed25519_signing.pub)"
```

## Verified Commits

When Codeplane receives a push containing signed commits, it automatically verifies each commit's signature against the public keys on the author's account.

### Verification Statuses

| Status | Meaning |
|--------|---------|
| **Verified** | The signature is valid and matches a key on the committer's Codeplane account. |
...
```bash
curl https://api.codeplane.app/api/repos/owner/repo/changes/kxyz1234 \
  -H "Authorization: token codeplane_your_token"
```
...
| `unverified_key` | Signature is valid but the key is not associated with any Codeplane account |
...
curl https://api.codeplane.app/api/repos/owner/repo/bookmarks/main/protection \
  -H "Authorization: token codeplane_your_token"
```
...
```bash
curl https://api.codeplane.app/api/user/gpg-keys \
  -H "Authorization: token codeplane_your_token"
```
...
```bash
curl https://api.codeplane.app/api/user/gpg-keys/1 \
  -H "Authorization: token codeplane_your_token"
```
...
```bash
curl -X DELETE https://api.codeplane.app/api/user/gpg-keys/1 \
  -H "Authorization: token codeplane_your_token"
```
...
1. Confirm the signing key is uploaded to your Codeplane account:
   ```bash
   codeplane gpg-key list
   ```
2. For GPG: ensure the email on your GPG key matches a verified email on your Codeplane account.
3. For SSH: ensure the signing key is added via `codeplane ssh-key add`.
...
### Commits signed locally but show as unsigned on Codeplane

This happens when the public key corresponding to your signing key has not been uploaded to Codeplane. Upload it with `codeplane gpg-key add` (for GPG) or `codeplane ssh-key add` (for SSH).
