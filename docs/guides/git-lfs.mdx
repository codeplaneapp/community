---
title: Git LFS
description: Store large binary files with Git Large File Storage on Codeplane
---
...
## How LFS Works on Codeplane

Codeplane stores LFS objects in dedicated blob storage. Uploads and downloads use **signed URLs** and go directly to storage -- they are not proxied through the API server. This keeps transfers fast and avoids bottlenecking the API.
...
3. Codeplane generates short-lived signed URLs
4. Your client uploads the LFS objects directly to storage
5. Your client confirms the upload with Codeplane
6. Codeplane records the object metadata in the database
```

When someone clones or pulls, the reverse happens: Codeplane provides signed download URLs and the client fetches objects directly from storage.

## Setup

### Prerequisites

You need Git LFS installed locally. Since jj uses git as its storage backend, standard Git LFS tooling works with Codeplane repositories.
...
3. Git LFS uploads the file content to Codeplane (via signed URLs)
...
jj git clone git@codeplane.app:owner/repo.git
...
Codeplane provides LFS endpoints for programmatic access. These follow the Git LFS Batch API specification and are used automatically by the Git LFS client.
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/lfs/upload \
...
```json
{
  "upload_url": "https://storage.codeplane.app/lfs/owner/repo/sha256-abc123...?signature=...",
...
After uploading, confirm the upload so Codeplane records the object metadata.

**Request:**

```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/lfs/confirm \
  -H "Authorization: token codeplane_your_token" \
  -H "Content-Type: application/json" \
  -d '{
    "oid": "sha256:abc123...",
...
When pulling LFS objects, the Git LFS client requests download URLs through the standard LFS Batch API. Codeplane returns signed URLs for direct download.
