...
description: Configure the Codeplane Linear integration from the UI or the CLI
---

# Configure Linear

Codeplane can connect a Linear team to a Codeplane repository in two ways:

- **UI**: best when you want Codeplane to drive the OAuth flow and team discovery for you.
- **CLI**: best for scripted setup or when you already have the Linear team metadata and OAuth token.

## Configure Linear in the UI

1. Open **Integrations** in the Codeplane UI.
2. Open **Linear Sync**.
3. Click **Connect Linear**.
4. Complete the Linear OAuth flow.
5. Choose the Linear team and Codeplane repository.
6. Click **Connect Team**.

Codeplane now keeps the Linear OAuth credentials server-side during this flow. The browser only receives a short-lived setup handle plus the team metadata needed to finish configuration.
...
Use `codeplane extension linear install` when you already know the target repo and the Linear team you want to connect.

```bash
cat <<'EOF' | codeplane extension linear install \
  --team-id team_123 \
  --team-name "Platform" \
  --team-key PLT \
  --repo-owner alice \
...
codeplane extension linear sync 7

# Remove integration 7
codeplane extension linear remove 7
```
