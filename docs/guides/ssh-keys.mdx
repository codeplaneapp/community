---
title: SSH Keys
description: Set up SSH keys for secure, passwordless git push and pull with Codeplane
---

# SSH Keys

SSH keys provide secure, passwordless authentication for git operations (push, pull, clone) over SSH. When you add an SSH public key to your Codeplane account, Codeplane's SSH server matches the key fingerprint on every connection, letting you interact with any repository you have access to without entering a password or token.
...
- **Strong cryptography**: SSH keys use public-key cryptography. Your private key never leaves your machine -- only the public key is stored on Codeplane.
...
Codeplane accepts the following SSH key types:
...
If you already have an SSH key you want to use, skip to [Adding Your SSH Key to Codeplane](#adding-your-ssh-key-to-codeplane-via-cli).
...
- `~/.ssh/id_ed25519.pub` -- your public key (this is what you add to Codeplane)
...
## Adding Your SSH Key to Codeplane via CLI

Copy your public key and add it using the `codeplane ssh-key add` command:
...
You can also register SSH keys through the Codeplane API directly:

```bash
curl -X POST https://api.codeplane.app/api/user/keys \
...
```bash
curl https://api.codeplane.app/api/user/keys \
  -H "Authorization: token codeplane_your_token"
```
...
```bash
codeplane ssh-key delete 1
```

The positional argument is the key ID, which you can find from `codeplane ssh-key list`.

### Via API

```bash
curl -X DELETE https://api.codeplane.app/api/user/keys/1 \
  -H "Authorization: token codeplane_your_token"
```
...
After adding your key, verify that Codeplane recognizes it:

```bash
ssh -T ssh.codeplane.app
```

If your key is configured correctly, you will see a message like:

```
Hi alice! You've successfully authenticated with Codeplane.
```
...
```bash
# Using the codeplane CLI (auto-detects SSH or HTTPS based on your config)
codeplane repo clone owner/repo

# Using jj directly with an SSH URL
jj git clone ssh://ssh.codeplane.app/owner/repo
```
...
```bash
codeplane config set git_protocol ssh
```

This is the default. To switch to HTTPS instead:

```bash
codeplane config set git_protocol https
```

## Multiple SSH Keys

If you use multiple Codeplane accounts (for example, a personal account and a work account), or if you want to use different keys for different hosts, configure SSH to select the right key automatically.
...
```
# Personal Codeplane account
Host codeplane-personal
  HostName ssh.codeplane.app
  User git
  IdentityFile ~/.ssh/id_ed25519_personal
  IdentitiesOnly yes

# Work Codeplane account
Host codeplane-work
  HostName ssh.codeplane.app
  User git
...
```bash
jj git clone ssh://codeplane-personal/owner/repo
jj git clone ssh://codeplane-work/org/repo
```
...
```bash
ssh -T -i ~/.ssh/id_ed25519_work ssh.codeplane.app
```
...
This means Codeplane's SSH server did not accept any of the keys your SSH client offered.

**Check that your key is added to Codeplane:**
...
```bash
codeplane ssh-key add --title "My Key" --key "$(cat ~/.ssh/id_ed25519.pub)"
```
...
If you have multiple keys and SSH is offering the wrong one, use an SSH config file to specify which key to use for Codeplane:

```
Host ssh.codeplane.app
  IdentityFile ~/.ssh/id_ed25519
  IdentitiesOnly yes
```
...
Codeplane accepts Ed25519, ECDSA, and RSA (2048+ bits) keys. If you receive a `422` error when adding a key, check:
...
If `ssh -T ssh.codeplane.app` hangs without responding:
...
- Verify DNS resolution: `nslookup ssh.codeplane.app` should return an IP address.
...
| **CODEPLANE_TOKEN** | Scoped by token permissions | HTTPS (CLI API calls) | CI environments without SSH |
