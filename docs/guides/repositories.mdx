---
title: Repositories
description: Create, manage, and configure Codeplane repositories
icon: code-branch
---

# Repositories

Repositories are the core unit of Codeplane. They store your code, track changes via jj, and host landing requests, issues, workflows, and more. Codeplane repositories are jj-native -- they use bookmarks instead of branches, stable Change IDs instead of commit SHAs, and first-class conflict tracking.
...
codeplane repo create my-project --description "A new project"

# Create a private repository
codeplane repo create my-project --private

# Create a repository under an organization
codeplane repo create my-org/my-project --description "Org repository"
```
...
curl -X POST https://api.codeplane.app/api/user/repos \
...
curl -X POST https://api.codeplane.app/api/orgs/my-org/repos \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-project",
...
```bash
# Clone by owner/repo
codeplane repo clone alice/my-project

# Clone into a specific directory
codeplane repo clone alice/my-project ./local-dir
```

The `codeplane repo clone` command clones via your preferred git protocol (SSH or HTTPS, configurable with `codeplane config set git_protocol`). After cloning, you work with standard jj commands.
...
codeplane repo list --archived

# View repository details
codeplane repo view
codeplane repo view -R alice/my-project

# JSON output
codeplane repo view --json
```

## Bookmarks

Bookmarks are jj's replacement for git branches. They are named pointers to changes, and they are the primary reference type in Codeplane.
...
codeplane bookmark list --remote

# Create a bookmark at the current working copy
codeplane bookmark create feature-auth

# Create a bookmark at a specific change
codeplane bookmark create feature-auth --change-id kxyz1234

# Delete a bookmark
codeplane bookmark delete feature-auth

# Remote operations
codeplane bookmark create feature-auth --remote --change-id kxyz1234 -R owner/repo
codeplane bookmark delete feature-auth --remote -R owner/repo
```
...
codeplane change show kxyz1234

# Show the diff for a change
codeplane change diff kxyz1234
...
```bash
curl -X PATCH https://api.codeplane.app/api/repos/owner/repo \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{"private": true}'
```
...
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/owner/repo/topics

# Set topics
curl -X PUT https://api.codeplane.app/api/repos/owner/repo/topics \
  -H "Authorization: token codeplane_xxxxx" \
  -H "Content-Type: application/json" \
...
```bash
# Star a repository
curl -X PUT -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/user/starred/owner/repo

# Unstar a repository
curl -X DELETE -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/user/starred/owner/repo

# List stargazers
curl https://api.codeplane.app/api/repos/owner/repo/stargazers
```
...
codeplane repo transfer alice/my-repo acme-org

# Skip confirmation prompt
codeplane repo transfer alice/my-repo acme-org -y
```
...
```bash
# List deploy keys
codeplane repo deploy-key list -R owner/repo

# Add a read-only deploy key
codeplane repo deploy-key add -R owner/repo \
  --title "CI Runner" \
...
# Delete a deploy key
codeplane repo deploy-key delete -R owner/repo 3
```
...
# Get file contents
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/owner/repo/contents/src/main.ts

# Get a git tree
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/owner/repo/git/trees/main
```
...
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/owner/repo/operations
```
