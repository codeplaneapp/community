...
Codeplane workspaces run on cloud microVMs. The public workspace API is now first-class: you create a workspace, connect over SSH, suspend or resume it, fork it, and snapshot it without going through the older session-only surface.
...
```bash
curl -X POST https://api.codeplane.app/api/repos/myorg/myrepo/workspaces \
...
```json
{
  ...
  "vm_id": "vm_abc123",
  "persistence": "sticky",
  "ssh_host": "vm_abc123@vm-ssh.codeplane.app",
...
```bash
curl https://api.codeplane.app/api/repos/myorg/myrepo/workspaces/6ffb7076-08d8-438c-8f77-5d9f6c31d1e1/ssh \
  -H "Authorization: token codeplane_xxx"
```
...
```json
{
  ...
  "vm_id": "vm_abc123",
  "host": "vm-ssh.codeplane.app",
  "ssh_host": "vm_abc123+developer@vm-ssh.codeplane.app",
  "username": "developer",
  "port": 22,
  "access_token": "ws_xxx",
  "command": "ssh vm_abc123+developer:ws_xxx@vm-ssh.codeplane.app"
}
```
...
codeplane workspace ssh --repo myorg/myrepo

# Inspect the current workspace and SSH command
codeplane workspace view 6ffb7076-08d8-438c-8f77-5d9f6c31d1e1 --repo myorg/myrepo
```

There is also an issue-focused shortcut that bootstraps a workspace and draft landing request together:

```bash
codeplane workspace issue 42 --repo myorg/myrepo --target main
```

`codeplane workspace issue` fetches the issue, creates a workspace, opens a draft landing request that references the issue, and then connects to the workspace so you can start working immediately.
...
```bash
curl -X POST https://api.codeplane.app/api/repos/myorg/myrepo/workspaces/6ffb7076-08d8-438c-8f77-5d9f6c31d1e1/suspend \
  -H "Authorization: token codeplane_xxx"
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/myorg/myrepo/workspaces/6ffb7076-08d8-438c-8f77-5d9f6c31d1e1/resume \
  -H "Authorization: token codeplane_xxx"
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/myorg/myrepo/workspaces/6ffb7076-08d8-438c-8f77-5d9f6c31d1e1/fork \
  -H "Authorization: token codeplane_xxx" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://api.codeplane.app/api/repos/myorg/myrepo/workspaces/6ffb7076-08d8-438c-8f77-5d9f6c31d1e1/snapshot \
  -H "Authorization: token codeplane_xxx" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://api.codeplane.app/api/repos/myorg/myrepo/workspace-snapshots \
  -H "Authorization: token codeplane_xxx"
```
...
Use `/workspaces` for new integrations and the `codeplane workspace ...` CLI surface.
