...
- **No user account required**: Automated systems do not need their own Codeplane user account. A deploy key is registered directly on the repository.
...
ssh-keygen -t ed25519 -f ~/.ssh/codeplane_deploy -N "" -C "deploy@prod"

# Register the key
codeplane repo deploy-key add -R owner/repo \
  --title "Production Deploy" \
  --key "$(cat ~/.ssh/codeplane_deploy.pub)" \
  --read-only
```
...
codeplane repo deploy-key add -R owner/repo \
  --title "Mirror Sync" \
  --key "$(cat ~/.ssh/codeplane_mirror.pub)" \
  --read-only
```
...
```bash
codeplane repo deploy-key delete -R owner/repo 1
```
...
The positional argument is the deploy key ID, which you can find from `codeplane repo deploy-key list`.
...
```bash
curl https://api.codeplane.app/api/repos/owner/repo/keys \
...
```bash
curl https://api.codeplane.app/api/repos/owner/repo/keys/1 \
  -H "Authorization: token codeplane_your_token"
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/owner/repo/keys \
  -H "Authorization: token codeplane_your_token" \
...
```bash
curl -X DELETE https://api.codeplane.app/api/repos/owner/repo/keys/1 \
  -H "Authorization: token codeplane_your_token"
```
