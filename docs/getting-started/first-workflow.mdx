...
Codeplane workflows are TSX files stored in your repository under `.codeplane/workflows/`. They use [Smithers](https://smithers.sh) JSX components wrapped by the `@codeplane-ai/workflow` package.

## Project Setup

The `.codeplane/` directory at your repository root is a Bun workspace member with its own `package.json` and `tsconfig.json`. This gives you full LSP support (autocomplete, type checking, go-to-definition) when editing workflows.

### `.codeplane/package.json`

```json
{
  "name": "codeplane-workflows",
  "private": true,
  "type": "module",
  "dependencies": {
...
```json
{
  "compilerOptions": {
    ...
    "types": ["bun-types"],
    "paths": {
      "@codeplane-ai/workflow": ["../packages/workflow/src/index.ts"]
    }
  },
...
Add `.codeplane` and `packages/*` to your Bun workspaces so the `@codeplane-ai/workflow` dependency resolves:

```json
{
  "workspaces": ["packages/*", ".codeplane"]
}
```

Then run `bun install` to link the workspace.

The `@codeplane-ai/workflow` package lives at `packages/workflow/` and re-exports [Smithers](https://smithers.sh) JSX components (`Workflow`, `Task`, `Parallel`, `Sequence`, `Branch`, `Ralph`) plus the `on` trigger builders.
...
```bash
codeplane workflow run <workflow-id> --ref main
```
...
codeplane workflow run deploy --input environment=staging --input version=1.2.3

# Deploy to production with a dry run
codeplane workflow run deploy \
  --input environment=production \
  --input version=2.0.0 \
  --input dry_run=true

# Short form
codeplane workflow run deploy -i environment=staging -i version=1.2.3
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/alice/my-project/workflows/deploy/dispatches \
...
codeplane run list <workflow-id>

# View a specific run
codeplane run view <run-id>

# Watch a run in real-time
codeplane run watch <run-id>

# Re-run a workflow
codeplane run rerun <run-id>
```
...
```tsx
// .codeplane/workflows/nightly-build.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
import { $ } from "bun";
...
```tsx
// .codeplane/workflows/maintenance.tsx
import { Workflow, Task, on } from "@codeplane-ai/workflow";
...
Scheduled workflows are managed by the Codeplane scheduler on the server side, not triggered by push events or user actions. When a schedule fires:
...
```tsx
// .codeplane/workflows/bugfix.tsx
