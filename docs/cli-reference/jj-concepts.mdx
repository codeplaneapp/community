...
description: Understanding Jujutsu (jj) concepts as used by Codeplane
---

# jj Concepts Guide

Codeplane is built natively on [Jujutsu (jj)](https://github.com/jj-vcs/jj), a next-generation VCS. This guide explains key jj concepts.
...
- Codeplane tracks and displays Change IDs everywhere
...
Codeplane's premiere feature is first-class support for stacked changes:
...
Codeplane uses "Landing Requests" (LRs) instead of "Pull Requests". LRs natively understand stacked changes and Change IDs.

```bash
codeplane land create --title "Add feature X"
```
