...
The Codeplane API enforces per-user rate limits to ensure fair usage and platform stability. Every API response includes rate limit headers so you can monitor your consumption in real time.
...
These headers follow the same format used by Gitea and GitHub, so existing tooling that reads rate limit headers will work with Codeplane.

## Example: Normal Response Headers

```bash
curl -i https://api.codeplane.app/api/user \
...
```bash
curl -s -I https://api.codeplane.app/api/user \
  -H "Authorization: Bearer codeplane_your_token" \
  | grep X-RateLimit
```
