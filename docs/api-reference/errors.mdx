...
The Codeplane API uses a consistent, Gitea-compatible error format across all endpoints. Every error response is a JSON object with a human-readable `message` and an optional `errors` array containing field-level details.
...
```bash
curl -X POST https://api.codeplane.app/api/repos/alice/my-repo/issues \
...
```bash
curl https://api.codeplane.app/api/repos/alice/nonexistent \
  -H "Authorization: Bearer codeplane_your_token"
```
...
```bash
curl https://api.codeplane.app/api/user \
  -H "Authorization: Bearer codeplane_invalid_token"
```
...
```bash
curl -X DELETE https://api.codeplane.app/api/repos/bob/private-repo \
  -H "Authorization: Bearer codeplane_your_token"
```
...
```bash
curl -X POST https://api.codeplane.app/api/repos/alice/my-repo/landings \
  -H "Authorization: Bearer codeplane_your_token" \
  -H "Content-Type: application/json" \
  -d '{"target_bookmark": ""}'
```
