...
All list endpoints in the Codeplane API return paginated results using **cursor-based** pagination. Each response includes a `next_cursor` field in the body and a `Link` header for navigation.
...
curl "https://api.codeplane.app/api/repos/owner/repo/issues" \
  -H "Authorization: token codeplane_xxxxx"

# 50 results per page
curl "https://api.codeplane.app/api/repos/owner/repo/issues?limit=50" \
  -H "Authorization: token codeplane_xxxxx"

# Next page using cursor from previous response
curl "https://api.codeplane.app/api/repos/owner/repo/issues?limit=50&cursor=eyJpZCI6MTAwfQ" \
  -H "Authorization: token codeplane_xxxxx"
```
...
```
Link: <https://api.codeplane.app/api/repos/owner/repo/issues?limit=30&cursor=eyJpZCI6NjB9>; rel="next",
      <https://api.codeplane.app/api/repos/owner/repo/issues?limit=30>; rel="first"
```
...
```bash
curl -i "https://api.codeplane.app/api/repos/alice/my-project/issues?limit=10" \
  -H "Authorization: token codeplane_xxxxx"
```
...
Link: <https://api.codeplane.app/api/repos/alice/my-project/issues?limit=10&cursor=eyJpZCI6MTB9>; rel="next",
      <https://api.codeplane.app/api/repos/alice/my-project/issues?limit=10>; rel="first"
```
...
curl -s "https://api.codeplane.app/api/repos/owner/repo/issues?limit=10" \
  -H "Authorization: token codeplane_xxxxx"
```
...
async function fetchAllIssues(owner: string, repo: string, token: string) {
  ...
  while (true) {
    ...
    const response = await fetch(
      `https://api.codeplane.app/api/repos/${owner}/${repo}/issues?${params}`,
...
The `codeplane` CLI handles pagination automatically. Use `--limit` (short: `-L`) to control how many results are returned. The default is 30 and the maximum is 100, consistent across all commands.
...
# Fetch up to 100
codeplane land list --limit 100

# Works the same for all list commands
codeplane workflow list --limit 50
codeplane run list ci --limit 20
```
