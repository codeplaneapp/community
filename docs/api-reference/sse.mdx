...
description: Consume real-time event streams from the Codeplane API
---

# Server-Sent Events (SSE)

Codeplane uses **Server-Sent Events** (SSE) for all real-time features. SSE provides a lightweight, HTTP-based protocol for streaming events from the server to connected clients. Codeplane chose SSE over WebSockets or polling because it works naturally with HTTP infrastructure, supports automatic reconnection, and is well-suited for streaming use cases like workflow logs and agent output.
...
```bash
curl -N \
  -H "Authorization: Bearer codeplane_your_token_here" \
  -H "Accept: text/event-stream" \
  https://api.codeplane.app/api/events/stream
```
...
In addition to the unified stream, Codeplane provides resource-specific SSE endpoints for targeted use cases:
...
curl -N \
  -H "Authorization: Bearer codeplane_your_token_here" \
  "https://api.codeplane.app/api/events/stream?channels=notification,repo.push"
```
...
curl -N \
  -H "Authorization: Bearer codeplane_your_token_here" \
  "https://api.codeplane.app/api/events/stream?channels=landing_request.update&repo=alice/my-repo"
```
...
curl -N \
  ...
  "https://api.codeplane.app/api/events/stream?channels=workflow.log"
```
...
curl -N \
  -H "Authorization: Bearer codeplane_your_token_here" \
  https://api.codeplane.app/api/repos/alice/my-repo/runs/5/logs
```
...
await fetchEventSource("https://api.codeplane.app/api/events/stream?channels=notification", {
  headers: {
    "Authorization": "Bearer codeplane_your_token_here",
  },
  onmessage(event) {
...
The `codeplane` CLI uses SSE internally. The `codeplane run logs` command (formerly `watch`) streams workflow logs in real time:

```bash
# Watch a workflow run
codeplane run logs 5
```
