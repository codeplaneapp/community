...
Webhooks allow you to build or set up integrations which subscribe to certain events on Codeplane. When one of those events is triggered, we'll send a HTTP POST payload to the webhook's configured URL. Webhooks can be used to update an external issue tracker, trigger CI builds, update a backup mirror, or even deploy to your production server.
...
```bash
curl -X POST https://api.codeplane.app/api/repos/alice/my-project/hooks \
...
Codeplane currently supports the following event types:
...
If a `secret` is configured, Codeplane signs every delivery with an HMAC-SHA256 signature. The signature is sent in the `X-Codeplane-Signature-256` header.
...
Codeplane includes several headers in every delivery to help you identify and process the event:

| Header | Description |
|--------|-------------|
| `X-Codeplane-Event` | The name of the event type (e.g., `push`). |
| `X-Codeplane-Delivery` | A unique ID for the delivery. |
| `X-Codeplane-Signature-256` | The HMAC-SHA256 signature (if secret is set). |
| `User-Agent` | Always starts with `Codeplane-Hookshot/`. |

## Retries

If your server responds with a non-2xx status code, Codeplane will attempt to redeliver the payload up to **5 times** with exponential backoff.
