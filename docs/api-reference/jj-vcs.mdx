---
title: jj VCS API
description: jj-native version control operations - bookmarks, changes, diffs, and operation log via the Codeplane API
---

<Tip>
  These endpoints expose jj-native VCS operations that have no direct GitHub equivalent. They are proxied through the Codeplane API to the Rust repo-host which uses jj-lib natively.
</Tip>

# jj VCS API

Codeplane is a **jj-native** platform. All version control operations use jj concepts: Change IDs, Bookmarks, Operation Logs, and stacked changes. These endpoints provide direct access to jj VCS operations on repositories.
...
Codeplane's premiere feature is first-class support for **stacked changes** - a series of changes where each builds on the previous. This enables incremental review and is particularly powerful for AI-assisted development.
...
Commit statuses let external systems (CI/CD pipelines, code quality tools, security scanners) report the outcome of checks against a specific commit. Statuses are the mechanism by which Codeplane knows whether a change has passed CI, failed a linter, or is still being validated.

Statuses are the foundation for two key features:
- **`codeplane land checks`** -- shows the CI status of all changes in a landing request
...
```bash
curl -X POST https://api.codeplane.app/api/repos/alice/my-project/statuses/abc123def456 \
  ...
  -d '{
    ...
    "target_url": "https://api.codeplane.app/alice/my-project/runs/87"
  }'
```
...
curl -H "Authorization: token codeplane_xxxxx" \
  https://api.codeplane.app/api/repos/alice/my-project/commits/abc123def456/statuses
```
...
[
  ...
  },
  {
    ...
    "target_url": "https://api.codeplane.app/alice/my-project/runs/88",
    "creator": { "login": "alice", "id": 1 },
...
When multiple statuses exist on a commit, Codeplane computes a **combined status** using these rules:
...
The `codeplane land checks` command displays both individual statuses and the combined status:
...
When a Codeplane workflow runs against a commit, the platform automatically manages commit statuses on behalf of the workflow:

1. **Workflow starts** -- Codeplane creates a `pending` status with `context` set to the workflow name (e.g., `ci/test`). The `target_url` points to the workflow run's log page.
2. **Workflow completes successfully** -- Codeplane updates the status to `success` with a description summarizing the result.
3. **Workflow fails** -- Codeplane updates the status to `failure` (if the workflow's steps failed) or `error` (if the runner itself had a problem).

This means most users never need to call the status API directly -- their workflows create and update statuses automatically. The API is available for external CI/CD systems, third-party integrations, and custom tooling that runs outside of Codeplane's workflow system.

**Example workflow that implicitly sets a status:**

```tsx
// .codeplane/workflows/ci.tsx
...
When this workflow triggers, Codeplane creates a `pending` status with context `ci/test` on the triggering commit. When `bun test` succeeds, the status updates to `success`. If it fails, the status updates to `failure`.
...
2. **Attempt to land a change:** When `codeplane land land` is called, Codeplane checks the combined status of every commit in the landing request's change stack.
...
codeplane status list abc123def456
codeplane status list main
codeplane status list wqnwkozp

# Set a status on a specific commit SHA
codeplane status set abc123def456 --state success --context ci/test
codeplane status set abc123def456 --state failure --context ci/lint --description "3 errors"
codeplane status set abc123def456 --state pending --context ci/build --target-url "https://..."
...
When a commit status changes, Codeplane fires a `status` webhook event. This lets external systems react to CI outcomes -- for example, sending a notification when a previously-failing check turns green.
