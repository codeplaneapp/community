...
import type { AuthSource } from "../providers/AuthProvider.js";

interface AuthErrorScreenProps {
  variant: "no-token" | "expired";
  host: string;
  tokenSource: AuthSource | null;
  onRetry: () => void;
}
...
export function AuthErrorScreen({ variant, host, tokenSource, onRetry }: AuthErrorScreenProps) {
  ...
  useKeyboard((event) => {
    ...
    if (event.name === "r" && event.shift) { // Handle uppercase R
      handleRetry();
  ...
  return (
    <box flexDirection="column" width="100%" height="100%">
      ...
      <box flexDirection="column" flexGrow={1} justifyContent="center" paddingX={2}>
        <text fg={theme.error}>✗ Session expired</text>
        <text />
