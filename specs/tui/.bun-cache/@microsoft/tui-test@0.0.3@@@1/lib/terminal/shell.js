...
export const defaultShell = os.platform() == "win32"
    ? Shell.Cmd
    : os.platform() == "darwin"
        ? Shell.Zsh
        : Shell.Bash;
...
export const shellLaunch = async (shell) => {
    const platform = os.platform();
    let shellTarget = shell == Shell.Bash && platform == "win32"
        ? await gitBashPath()
        : platform == "win32"
            ? `${shell}.exe`
            : shell;
    ...
    switch (shell) {
        ...
        case Shell.Fish:
            ...
            break;
        case Shell.Xonsh: {
            const sharedConfig = os.platform() == "win32"
