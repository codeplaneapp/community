...
const transformFile = async (source, sourceContent, sourceHash, destination) => {
...
const copyFilesToCache = async (directory, destination) => {
    ...
    await Promise.all(directoryItems.map(async (directoryItem) => {
        ...
        if (directoryItem.isDirectory() && directoryItem.name == "node_modules") {
            ...
            if (typescriptPattern.test(fileExtension) ||
                javascriptPattern.test(fileExtension)) {
