...
import { getRootSuite } from "../test/suite.js";
import { transformFiles } from "./transform.js";
import { getRetries, getShellReadyTimeout, getTimeout, loadConfig, } from "../config/config.js";
...
const createWorkerPool = (maxWorkers) => {
...
const runSuites = async (allSuites, filteredTestIds, reporter, options, config, pool) => {
    ...
    while (suites.length != 0) {
        ...
        if (!suite)
            break;
        tasks.push(...suite.tests.map(async (test) => {
            if (!filteredTestIds.has(test.id)) {
                return;
            }
            for (let i = 0; i < Math.max(0, getRetries()) + 1; i++) {
                const testResult = await runTestWorker(test, test.sourcePath(), {
                    timeout: getTimeout(),
                    updateSnapshot,
                    shellReadyTimeout: getShellReadyTimeout(),
                }, trace, pool, reporter, i, config.traceFolder);
...
const checkRuntimeVersion = () => {
    if (isBun()) {
        if (os.platform() === "win32") {
            console.error(chalk.red(`Error: ${executableName} does not support Bun on Windows. Please use Node.js instead.\n`));
            process.exit(1);
        }
        if (Bun.version.localeCompare("1.3.5", undefined, { numeric: true }) < 0) {
            console.warn(chalk.yellow(`Warning: ${executableName} works best with Bun version 1.3.5 or higher (you are using ${Bun.version}).\n`));
        }
        return;
    }
    ...
    if (nodeMajorVersion != "16" &&
        ...
        nodeMajorVersion != "22" &&
        nodeMajorVersion != "24") {
        console.warn(chalk.yellow(`Warning: ${executableName} works best when using a supported node versions (which ${nodeVersion} is not).\n`));
    }
};
const checkShellSupport = (shells) => {
    let platform = "";
    let badShells = [];
    if (os.platform() === "darwin") {
        badShells = shells.filter((shell) => shell == Shell.Cmd ||
            shell == Shell.Powershell ||
            shell == Shell.WindowsPowershell);
        platform = "macOS";
    }
    else if (os.platform() == "win32") {
        badShells = shells.filter((shell) => shell == Shell.Zsh || shell == Shell.Fish);
        platform = "Windows";
    }
    else if (os.platform() == "linux") {
        badShells = shells.filter((shell) => shell == Shell.Cmd || shell == Shell.WindowsPowershell);
        platform = "Linux";
    }
    if (badShells.length != 0) {
        console.warn(chalk.yellow(`Warning: ${executableName} does not support the following shells on ${platform} ${badShells.join(", ")}`));
    }
};
const cleanSnapshots = async (tests, { updateSnapshot }) => {
    ...
    for (const snapshotFile of snapshotFiles.keys()) {
        unusedSnapshots += await cleanSnapshot(snapshotFile, new Set(snapshotFiles.get(snapshotFile)), updateSnapshot);
    }
...
export const run = async (options) => {
    ...
    while (suites.length != 0) {
        ...
        if (importSuite?.type === "file") {
            const transformedSuitePath = path.join(process.cwd(), cacheFolderName, importSuite.title);
    ...
    if (options.testFilter != null && options.testFilter.length > 0) {
        try {
            const patterns = options.testFilter.map((filter) => new RegExp(filter.replaceAll("\\", "\\\\")));
            ...
            console.error("Error: invalid test filter supplied. Test filters must be valid regular expressions");
            process.exit(1);
        }
    }
    const shells = Array.from(new Set(allTests
    ...
    await runSuites(rootSuite.suites, new Set(allTests.map((test) => test.id)), reporter, options, config, pool);
    try {
