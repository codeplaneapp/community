...
Specification for WORKFLOW_RUN_CLI_RERUN.
...
From the CLI, a developer runs `codeplane workflow run rerun <id>` to trigger a rerun on any workflow run that has reached a terminal state. Rerun creates a brand-new workflow run using the same workflow definition, the same trigger event type, the same target ref and commit SHA, and the same dispatch inputs that the original run used. The new run is enqueued independently: it gets its own run ID, its own step graph, its own logs, and its own lifecycle. The original run remains unchanged in the run history, giving the developer a clear audit trail of what ran and when.
...
- [ ] The CLI supports both `--json` output and human-readable output
- [ ] The CLI supports `--repo OWNER/REPO` flag and falls back to current repo context from the working directory
...
- [ ] The CLI exits with code 0 on success and code 1 on any error
...
- [ ] The `--repo` flag must be in `OWNER/REPO` format; other formats produce a clear error message
...
- [ ] The original run's `dispatch_inputs` containing deeply nested objects (up to 10 levels), arrays, null values, or empty strings are preserved through JSON round-trip
- [ ] Unicode characters (including emoji, CJK, RTL text) in dispatch input values are preserved through rerun
- [ ] Running the CLI outside any repository directory without `--repo` produces a clear "No repository context" error
- [ ] Running the CLI with an empty string as the run ID produces a clear validation error
...
**Arguments:**
- `id` (required, positive integer): The workflow run ID to rerun. Parsed via `z.coerce.number()`.

**Options:**
- `--repo OWNER/REPO`: Target repository. Defaults to current repo context from working directory or config. Resolved via `resolveRepoRef()`.
- `--json`: Output full API response as JSON instead of human-readable format.
...
**Current implementation note:** The CLI currently calls `POST /api/repos/:owner/:repo/runs/:id/rerun` which does not match either the v1 (`/actions/runs/:id/rerun`) or v2 (`/workflows/runs/:id/rerun`) server route. This path must be updated to use the v2 path `/api/repos/:owner/:repo/workflows/runs/:id/rerun` to match the server route layer.
...
**Request Body:** Optional, currently ignored. Forward-compatible with a future schema that may allow overriding inputs or ref.
...
**Hook: `useWorkflowRunRerun(repo, callbacks?)`**

- Calls `POST /api/repos/:owner/:repo/workflows/runs/:id/rerun`
- Returns `{ execute: (runId: number) => Promise<WorkflowRunResult>, loading: boolean, error: HookError | null, reset: () => void }`
- Supports `onOptimistic`, `onSuccess`, and `onError` callbacks with rollback support
- The `execute` function is idempotent-safe: calling it multiple times creates multiple independent runs (intentional)
...
1. **CLI reference for `workflow run rerun`:** Full argument/option documentation with examples showing both `--json` and human-readable output, including the `--repo` flag and implicit repo context behavior
2. **Workflow Run Rerun concept page:** Explain what rerun does (creates a new run from the same context), how it differs from resume (fresh start vs. partial retry), and when to use each
3. **API reference:** OpenAPI-style documentation for both v1 and v2 rerun endpoints including request/response shapes and error codes
4. **FAQ entry:** "Can I rerun a run if the workflow file was changed?" — Yes, the rerun uses the current workflow definition, not a frozen snapshot
5. **FAQ entry:** "What happens to my dispatch inputs when I rerun?" — They are faithfully preserved from the original run
...
- All permission checks are enforced server-side regardless of client-side assumptions
- PAT-based auth and session cookie auth are both supported
...
- No server-side auto-retry or queuing — the client (CLI) must wait and re-initiate
...
- CLI output with `--json` may contain dispatch inputs; users should be cautious when piping JSON output to logs in shared CI environments
...
|------------|---------|------------|
| `WorkflowRunRerunInitiated` | User runs `codeplane workflow run rerun` | `repository_id`, `original_run_id`, `user_id`, `client: "cli"`, `original_status` (the status of the run being rerun) |
| `WorkflowRunRerunSucceeded` | API returns 201 | `repository_id`, `original_run_id`, `new_run_id`, `workflow_definition_id`, `user_id`, `client: "cli"`, `step_count`, `latency_ms` |
| `WorkflowRunRerunFailed` | API returns 4xx/5xx | `repository_id`, `original_run_id`, `user_id`, `client: "cli"`, `error_code`, `error_message` |

### Funnel Metrics

- **CLI rerun adoption rate:** % of terminal runs that are rerun via CLI at least once (target: measurable usage from automation-heavy teams)
- **Rerun success rate:** % of rerun API calls from CLI that return 201 (target: >99%)
...
- **Mean time from failure to rerun:** Average seconds between original run reaching terminal state and rerun being triggered via CLI (indicates developer response time and automation speed)
- **Rerun chain depth:** Distribution of how many times a single original run lineage is rerun (identifies persistent failures)
- **CLI vs. Web vs. TUI distribution:** Breakdown of rerun triggers by client surface to understand which surfaces are most used for reruns
- **Repeat rerun rate:** % of runs that are rerun more than once within 1 hour via CLI (may indicate systematic infrastructure issues or automation loops)
...
|--------|------|--------|-------------|
| `codeplane_workflow_rerun_total` | Counter | `repository_id`, `status` (success/error), `client` (cli/web/tui/api) | Total rerun requests |
...
| Rate limit exceeded | 429 response | User cannot rerun immediately | CLI shows retry-after guidance; per-user limit prevents abuse |
...
| CLI API path mismatch | CLI hits `/runs/:id/rerun` instead of `/workflows/runs/:id/rerun` | 404 or routing miss | Update CLI to use v2 path |

## Verification

### API Integration Tests

- [ ] **Rerun a successful run (v2 path):** Dispatch a workflow, wait for success, POST rerun to `/api/repos/:owner/:repo/workflows/runs/:id/rerun`, assert 201 with `workflow_run_id` different from original, `workflow_definition_id` matching, and non-empty `steps` array
...
- [ ] **Rerun rate limiting:** Send 61 rerun requests within 60 seconds — assert the 61st returns 429 with `Retry-After` header
...
- [ ] **CLI rerun with implicit repo context:** From a repository directory, `codeplane workflow run rerun <id> --json` — assert success with new run ID
- [ ] **CLI rerun of cancelled run:** Cancel a run, then `codeplane workflow run rerun <id> --json` — assert new run is created with independent ID
- [ ] **CLI rerun of failed run:** Create a failing run, then `codeplane workflow run rerun <id> --json` — assert 201 and new run
...
- [ ] **CLI rerun invalid ID (string):** `codeplane workflow run rerun abc` — assert error output and exit code 1
- [ ] **CLI rerun invalid ID (zero):** `codeplane workflow run rerun 0` — assert error output and exit code 1
- [ ] **CLI rerun invalid ID (negative):** `codeplane workflow run rerun -- -1` — assert error output and exit code 1
- [ ] **CLI rerun without repo context:** `codeplane workflow run rerun 1` (outside any repo, no `--repo`) — assert error about missing repository context and exit code 1
...
- [ ] **CLI full lifecycle test:** Register workflow → dispatch → view → cancel → rerun → verify new run appears in `workflow runs` list (matches existing `workflow-lifecycle.test.ts` pattern)
- [ ] **CLI rerun chain:** Rerun a run, then rerun the resulting run — assert both produce distinct new run IDs
- [ ] **CLI rerun multiple times:** Rerun the same run ID 3 times — assert 3 distinct new run IDs
- [ ] **CLI rerun with --repo flag format validation:** `codeplane workflow run rerun 1 --repo "invalid-format"` — assert clear error about repository format
