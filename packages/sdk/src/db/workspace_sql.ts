...
RETURNING id, repository_id, user_id, name, is_fork, parent_workspace_id, source_snapshot_id, freestyle_vm_id, status, last_activity_at, idle_timeout_secs, suspended_at, created_at, updated_at`;
...
SELECT id, repository_id, user_id, name, is_fork, parent_workspace_id, source_snapshot_id, freestyle_vm_id, status, last_activity_at, idle_timeout_secs, suspended_at, created_at, updated_at
FROM workspaces
WHERE id = $1`;
...
export async function updateWorkspaceExecutionInfo(sql: Sql, args: UpdateWorkspaceExecutionInfoArgs): Promise<UpdateWorkspaceExecutionInfoRow | null> {
    const rows = await sql.unsafe(updateWorkspaceExecutionInfoQuery, [args.id, args.freestyleVmId, args.status]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
VALUES ($1, $2, $3, $4, $5)
RETURNING id, repository_id, user_id, workspace_id, name, freestyle_snapshot_id, created_at, updated_at`;
...
export async function createWorkspaceSnapshot(sql: Sql, args: CreateWorkspaceSnapshotArgs): Promise<CreateWorkspaceSnapshotRow | null> {
    const rows = await sql.unsafe(createWorkspaceSnapshotQuery, [args.repositoryId, args.userId, args.workspaceId, args.name, args.freestyleSnapshotId]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
SELECT id, repository_id, user_id, workspace_id, name, freestyle_snapshot_id, created_at, updated_at
...
SELECT w.id, w.repository_id, w.user_id, w.name, w.is_fork, w.parent_workspace_id, w.source_snapshot_id, w.freestyle_vm_id, w.status, w.last_activity_at, w.idle_timeout_secs, w.suspended_at, w.created_at, w.updated_at
FROM workspaces w
